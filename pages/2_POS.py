...
with col_cart:
    with st.container(border=True):
        ...
        if st.session_state.cart:
            ...
            # Display Items with Remove Button in a SCROLLABLE container (Dynamic Height)
            index_to_remove = None
            
            # Use fixed height only if items > 4 to prevent whitespace
            if len(st.session_state.cart) > 4:
                cart_container = st.container(height=400, border=False)
            else:
            ...
            # Totals & Checkout Display (Fixed at Bottom)
            if st.session_state.cart:
